...
{
	EnsureTile(tile);
	auto CurrentMeshSection = tile->GetMeshSectionIndex();
	...
	for (int i = 0; i < mesh.Vertices.Num(); i++)
	{
		...
		VerticesWithThreeUvs[i] = NewVert;
	}

	tile->GetMeshComponent()->CreateMeshSection(CurrentMeshSection, VerticesWithThreeUvs, mesh.Triangles, false, EUpdateFrequency::Infrequent, ESectionUpdateFlags::CalculateNormalTangent);
	...
	//tile->GetMeshComponent()->CreateMeshSection(CurrentMeshSection, VerticesWithThreeUvs, mesh.Triangles, SectionBoundingBox, false,EUpdateFrequency::Infrequent, ESectionUpdateFlags::None);

	// todo: catch renderstateDirty crash that I faced once

	auto Material = _customizationService->GetSharedMaterial("Materials/M_Shaded_Color");
	if (Material)
	{
		tile->GetMeshComponent()->SetMaterial(tile->GetMeshSectionIndex(), Material);
	}
	tile->IncrementMeshSectionIndex();
}
